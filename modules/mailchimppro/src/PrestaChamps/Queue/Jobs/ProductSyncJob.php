...
class ProductSyncJob implements JobInterface
{
    ...
    public static function handleBatch(array $jobs, $shopId)
    {
        $productIds = [];

        foreach ($jobs as $job) {
            $jobData = json_decode($job['body'], true);
            if (!empty($jobData['productId'])) {
                $productIds[] = (int) $jobData['productId'];
            }
        }
        ...
        $productSyncCommand = new \PrestaChamps\MailchimpPro\Commands\ProductSyncCommand(
            \Context::getContext(),
        ...
        $productSyncCommand->setSyncMode(
            \PrestaChamps\MailchimpPro\Commands\ProductSyncCommand::SYNC_MODE_BATCH
        );

        $productSyncCommand->setMethod(
            \PrestaChamps\MailchimpPro\Commands\ProductSyncCommand::SYNC_METHOD_PUT
        );

        return $productSyncCommand->execute();
    }
